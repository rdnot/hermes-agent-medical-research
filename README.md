...
| **Tool Summary Display** | ✅ Before final answer | ✅ Ported (CLI only) | **P0** |
...
- **Raised `web_extract` result threshold from 100K → 500K** (`max_result_size_chars`).
...
- **Tool Summary Display**: Added a transparency feature that tracks and clearly lists all tool calls and their arguments (e.g., `search()`, `web_fetch()`) made during a turn after the final response (CLI only).
