...
| **Tool Summary Display** | ✅ Before final answer | ✅ Ported + CLI fix | **P0** |
...
### CLI Display (`cli.py`)
- **Tool Summary in Streaming Mode**: Fixed tool summary not appearing in the CLI when token streaming was active. The summary (generated by `run_agent.py`) was appended to `final_response` after streaming ended, but the CLI skipped re-rendering `final_response` to avoid double-printing streamed text — silently dropping the summary. Fixed by storing the raw summary in `result['tool_summary']` and printing it explicitly after the stream box closes. Applies to both normal streaming and TTS streaming paths. Gateway channels (Telegram, Discord, WhatsApp) were unaffected as they always read `final_response` directly.
