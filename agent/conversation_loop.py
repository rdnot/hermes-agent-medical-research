...
def _build_tools_summary(tool_calls):
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                if api_call_count >= agent._force_final_threshold:
                ...
                # ── Fork: Tool Summary Display (transparency) ──────────────
                # Store for display AFTER final_response is printed.
                agent._pending_tool_summary = ""
                if agent._current_turn_tool_calls:
                    tool_summary = _build_tools_summary(agent._current_turn_tool_calls)
                    if tool_summary:
                        agent._pending_tool_summary = tool_summary
    ...
    result = {
        ...
        "tool_summary": getattr(agent, '_pending_tool_summary', ''),  # CLI handles display separately
